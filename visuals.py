...
models = os.listdir(experiment)
colors = ['red','blue','green','yellow','pink','orange','aqua','purple','gray','black']
par = re.compile("::(?!.*::)")

def create_figure(model):
    model_new = re.sub('^(sfwrs|sfwl|sfwla|sf)','',model)
    meta = json.load(open(os.path.join(*[dataset,model_new,'meta.json'])))
    ...
    val_epochs = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/val_epochs.pickle']),'rb'))
    val_epochs = np.array(val_epochs)
    train_epochs = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/train_epochs.pickle']),'rb'))
    ...
    val_acc = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/val_accuracy.pickle']),'rb'))
    ...
    train_acc = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/train_accuracy.pickle']),'rb'))
	...
	#Loss Plots
    train_loss = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/train_loss.pickle']),'rb'))
    train_loss = np.array(train_loss)
    val_loss = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/val_loss.pickle']),'rb'))
	...
	#Learning Rate
    lr = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/learning_rate.pickle']),'rb'))	
    lr = np.array(lr)
    ...
    conf = pickle.load(open(os.path.join(*[experiment,model,'/datastreams/val_confusion_matrix.pickle'],'rb'))
    conf = conf[-1]
